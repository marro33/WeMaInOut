...
+ (WToast *)__createWithText:(NSString *)text {
    ...
    textLabel.textColor = ColorRGBA(255, 255, 255, 1);
    textLabel.numberOfLines = 0;
    textLabel.lineBreakMode = NSLineBreakByWordWrapping;
    
    CGRect frame = CGRectZero;
    CGSize sizeConstraint = CGSizeMake(width - 20.0f, MAXFLOAT);
    ...
    toast.backgroundColor = ColorRGBA(0, 0, 0, 0.8f);
    textLabel.text = text;
...
- (void)__flipViewAccordingToStatusBarOrientation {
	...
	if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"8.0")) {
        ...
        //y = (kScreenHeight-self.bounds.size.height-TABBAR_OFFSET)/2.0;
	}
	else {
		UIInterfaceOrientation orientation = [[UIApplication sharedApplication] statusBarOrientation];
		CGFloat angle = 0.0;
...
+ (void)showWithText:(NSString *)text duration:(NSInteger)duration {
	...
	[WToast showWithText:text duration:duration roundedCorners:NO];
}
...
+ (void)showWithImage:(UIImage *)image duration:(NSInteger)duration {
	[WToast showWithImage:image duration:duration roundedCorners:NO];
}
